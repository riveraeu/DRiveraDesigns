...
export default {
  ...
  computed: {
    ...
    srcset() {
      ...
      if (typeof this.widthOnScreenTablet === 'undefined' && typeof this.widthOnScreenSmartphone === 'undefined') {
        return this.srcscetSizes.map(
          (width) => {
            const finalWidth = this.getWidthAdaptedToWidthOnScreen(width, this.widthOnScreen)
        ...
        return this.getSrcsetSizes('desktop').map(
          (width) => {
            const finalWidth = this.getWidthAdaptedToWidthOnScreen(width, this.widthOnScreenTablet)
            return this.getImageUrlWithWidthAndHeight(finalWidth, this.getHeightFromWidth(finalWidth)) + ` ${finalWidth}w`
          }
        ).join(', ')
      }
    ...
    smartphoneSrcset() {
      return this.getSrcsetSizes('smartphone').map(
        (width) => {
          const finalWidth = this.getWidthAdaptedToWidthOnScreen(width, this.widthOnScreenSmartphone)
  ...
  methods: {
    ...
    getImageUrlWithWidthAndHeight(width, height) {
      return this.imageUrl.replace(this.widthPlaceholder, 'h_' + height).replace(this.heightPlaceholder, 'w_' + width)
    }
  }
}
</script>
